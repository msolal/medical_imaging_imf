...
class SchrodBridgeIMF(torch.nn.Module):
    ...
    def loss_forward(self,
        ...
        loss = loss_scaling * (
            x1 - x0 - self.sigma * torch.sqrt(t/(1-t)) * z - pred
        ).square().sum(0)
    ...
    def loss_backward(self,
        ...
        loss = loss_scaling * (
            x0 - x1 - self.sigma * torch.sqrt((1-t)/t) * z - pred
        ).square().sum(0)

        return loss.sum()


class JointIMF(SchrodBridgeIMF):
    ...
    def loss_consistency(
        ...
        loss = (
            pred_f + pred_b + self.sigma * z * (torch.sqrt((1-t)/t) + torch.sqrt(t/(1-t)))
        ).square().sum(0)
