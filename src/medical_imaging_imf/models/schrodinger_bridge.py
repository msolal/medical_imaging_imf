...
class SchrodBridgeIMF(torch.nn.Module):
    ...
    def sample_forward_sde(
        ...
        ts = torch.arange(self.n_timesteps, device=x0.device) / self.n_timesteps
        trajectories = []

        x = x0.clone()
        for i in range(self.n_timesteps):
            t = torch.ones(batch_size, device=x0.device) * ts[i]
    ...
    def sample_backward_sde(
        ...
        ts = 1 - torch.arange(self.n_timesteps, device=x1.device) / self.n_timesteps
        trajectories = []

        x = x1.clone()
        for i in range(self.n_timesteps):
            t = torch.ones(batch_size, device=x1.device) * ts[i]
    ...
    def loss_forward(
        ...
        pred = self.network_forward(t.squeeze(), x_t_01)

        loss = loss_scaling * (x1 - x0 - self.sigma * torch.sqrt(t / (1 - t)) * z - pred).square().mean().sum()
    ...
    def loss_backward(self, x0: torch.Tensor, x1: torch.Tensor, z: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        pred = self.network_backward(t.squeeze(), x_t_01)

        loss = loss_scaling * (x0 - x1 - self.sigma * torch.sqrt((1 - t) / t) * z - pred).square().mean().sum()
    ...
    def loss_consistency(self, x0: torch.Tensor, x1: torch.Tensor, z: torch.Tensor, t: torch.Tensor):
        ...
        pred_f = self.network_forward(t.squeeze(), x_t_01)
        pred_b = self.network_backward(t.squeeze(), x_t_01)

        return (pred_f + pred_b + self.sigma * z * (torch.sqrt((1 - t) / t) + torch.sqrt(t / (1 - t)))).square().mean().sum()
